...
func (r NTPResult) String() string {
	if r.Error != "" { return fmt.Sprintf("%s %s %s", utils.Colorize("✗", utils.ColorRed), utils.BoldText("NTP Hata:"), utils.Colorize(r.Error, utils.ColorRed)) }
	status := "unreachable"
	if r.Reachable { status = "reachable" }
...
func ScanNTP(target string, timeout time.Duration) NTPResult {
	...
	if err != nil { return NTPResult{Target: target, Error: err.Error()} }
	defer conn.Close()
	...
	req[0] = 0x23
	if _, err := conn.Write(req); err != nil { return NTPResult{Target: target, Error: err.Error()} }

	resp := make([]byte, 48)
	n, err := conn.Read(resp)
	if err != nil || n < 48 { return NTPResult{Target: target, Reachable: false, Error: "no response"} }
