...
func (r SIPResult) String() string {
	if r.Error != "" { return fmt.Sprintf("%s %s %s", utils.Colorize("✗", utils.ColorRed), utils.BoldText("SIP Hata:"), utils.Colorize(r.Error, utils.ColorRed)) }
	parts := []string{}
	if r.Server != "" { parts = append(parts, "Server="+r.Server) }
	if r.UserAgent != "" { parts = append(parts, "User-Agent="+r.UserAgent) }
...
func ScanSIP(target string, timeout time.Duration) SIPResult {
	...
	if err != nil { return SIPResult{Target: target, Error: err.Error()} }
	defer conn.Close()
	...
	if _, err := conn.Write([]byte(msg)); err != nil { return SIPResult{Target: target, Error: err.Error()} }
	buf := make([]byte, 1500)
	n, err := conn.Read(buf)
	if err != nil { return SIPResult{Target: target, Reachable: false} }
	resp := string(buf[:n])
