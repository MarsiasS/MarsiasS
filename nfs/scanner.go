...
func writeOpaque(buf *bytes.Buffer, data []byte) {
	...
	buf.Write(data)
	pad := (4 - (len(data) % 4)) % 4
	if pad != 0 { buf.Write(make([]byte, pad)) }
}
...
func (x *xdrReader) u32() (uint32, error) {
	var v uint32
	if err := binary.Read(x, binary.BigEndian, &v); err != nil { return 0, err }
	return v, nil
}

func (x *xdrReader) bool() (bool, error) {
	v, err := x.u32(); if err != nil { return false, err }
	return v != 0, nil
}

func (x *xdrReader) opaque() ([]byte, error) {
	l, err := x.u32(); if err != nil { return nil, err }
	if l > 1<<20 { return nil, errors.New("xdr string too large") }
	data := make([]byte, l)
	if _, err := x.Read(data); err != nil { return nil, err }
	pad := (4 - (int(l) % 4)) % 4
	if pad > 0 { if _, err := x.Seek(int64(pad), 1); err != nil { return nil, err } }
	return data, nil
}
...
func (x *xdrReader) str() (string, error) {
	b, err := x.opaque(); if err != nil { return "", err }
	return string(b), nil
}
...
func rpcbindDump(conn net.Conn, timeout time.Duration) ([]ProgramMapping, error) {
	...
	if _, err := conn.Write(payload); err != nil { return nil, err }
	reply, err := readRPCReply(conn, timeout)
	if err != nil { return nil, err }

	// Parse RPC reply header
	x := newXDR(reply)
	if _, err := x.u32(); err != nil { return nil, err } // xid
	mtype, err := x.u32(); if err != nil || mtype != rpcReplyMsg { return nil, errors.New("not a reply") }
	// reply body: MSG_ACCEPTED path
	// skip reply state verifier etc
	// reply state
	_, err = x.u32(); if err != nil { return nil, err } // reply stat (0 accepted / 1 denied)
	// verifier
	if _, err := x.u32(); err != nil { return nil, err }
	if err := x.skipOpaque(); err != nil { return nil, err }
	// accept stat
	astat, err := x.u32(); if err != nil { return nil, err }
	if astat != 0 { return nil, errors.New("rpc accept not success") }

	// result: pmaplist (linked list via bool)
	var out []ProgramMapping
	for {
		more, err := x.bool(); if err != nil { return nil, err }
		if !more { break }
		prog, err := x.u32(); if err != nil { return nil, err }
		vers, err := x.u32(); if err != nil { return nil, err }
		proto, err := x.u32(); if err != nil { return nil, err }
		port, err := x.u32(); if err != nil { return nil, err }
...
func mappingsForProgram(all []ProgramMapping, program uint32) []ProgramMapping {
	...
	for _, m := range all { if m.Program == program { res = append(res, m) } }
	return res
}
...
func fetchExports(target string, port int, timeout time.Duration) ([]ExportInfo, bool, error) {
	...
	payload := buildRPCCall(call, authNULL, nil, authNULL, nil, nil)
	if _, err := conn.Write(payload); err != nil { return nil, false, err }
	reply, err := readRPCReply(conn, timeout)
	if err != nil { return nil, false, err }

	x := newXDR(reply)
	if _, err := x.u32(); err != nil { return nil, false, err } // xid
	mtype, err := x.u32(); if err != nil || mtype != rpcReplyMsg { return nil, false, errors.New("not a reply") }
	_, err = x.u32(); if err != nil { return nil, false, err } // reply stat
	// verifier
	if _, err := x.u32(); err != nil { return nil, false, err }
	if err := x.skipOpaque(); err != nil { return nil, false, err }
	astat, err := x.u32(); if err != nil { return nil, false, err }
	if astat != 0 { return nil, false, errors.New("rpc accept not success") }

	// result: export list (linked list)
	var exports []ExportInfo
	for {
		more, err := x.bool(); if err != nil { return nil, true, err }
		if !more { break }
		path, err := x.str(); if err != nil { return nil, true, err }
		var groups []string
		for {
			gmore, err := x.bool(); if err != nil { return nil, true, err }
			if !gmore { break }
			gname, err := x.str(); if err != nil { return nil, true, err }
...
// String renders result
func (r *NFSResult) String() string {
	if r.ErrorMessage != "" {
		return fmt.Sprintf("NFS %s - Error: %s", r.Target, r.ErrorMessage)
	}
	var b strings.Builder
	fmt.Fprintf(&b, "Target: %s\n", r.Target)
	fmt.Fprintf(&b, "rpcbind: %v\n", r.PortmapperAlive)
	if len(r.NFSPrograms) > 0 {
		fmt.Fprintf(&b, "NFS versions: %s\n", r.VersionSummary)
	}
	if r.MountdPort > 0 {
		fmt.Fprintf(&b, "mountd port: %d\n", r.MountdPort)
	}
	fmt.Fprintf(&b, "Exports: %d\n", len(r.Exports))
	for _, e := range r.Exports {
		fmt.Fprintf(&b, "- %s (groups: %s)\n", e.Path, strings.Join(e.Groups, ","))
	}
	fmt.Fprintf(&b, "Anonymous access: %v\n", r.Anonymous)
	return b.String()
}