...
func (r IPMIResult) String() string {
	if r.Error != "" { return fmt.Sprintf("%s %s %s", utils.Colorize("✗", utils.ColorRed), utils.BoldText("IPMI Hata:"), utils.Colorize(r.Error, utils.ColorRed)) }
...
func ScanIPMI(target string, timeout time.Duration) IPMIResult {
	...
	if err != nil { return IPMIResult{Target: target, Error: err.Error()} }
	defer conn.Close()
	...
	if _, err := conn.Write(req); err != nil { return IPMIResult{Target: target, Error: err.Error()} }
	buf := make([]byte, 64)
	n, err := conn.Read(buf)
	if err != nil || n < 12 { return IPMIResult{Target: target, Reachable: false, ASFPresent: false} }
