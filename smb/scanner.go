...
func (r *SMBResult) String() string {
	if r.ErrorMessage != "" {
		return fmt.Sprintf("SMB %s:%d - Error: %s", r.Target, r.Port, r.ErrorMessage)
	}
	...
	return fmt.Sprintf("SMB %s:%d - Version: %s (signing:%s required:%v) (SMBv1:%s)%s",
		r.Target, r.Port, r.Version, signingStr, r.SigningRequired, smbv1Str,
		func() string {
			if len(r.Dialects) > 0 {
				return fmt.Sprintf(" (dialects: [%s])", strings.Join(r.Dialects, ", "))
			}
			return ""
		}())
}
